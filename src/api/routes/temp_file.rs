use crate::api::controllers::temp_file::{
    delete_temp_thumbnail, delete_temp_video, get_input_template, get_temp_file,
    post_temp_thumbnail, post_temp_video,
};
use actix_web::web;
...
pub fn temp_file_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/temp")
            ...
            .route("/video/{temp_file}", web::delete().to(delete_temp_video))
            .route(
                "/thumbnail/{temp_file}",
                web::delete().to(delete_temp_thumbnail),
            )
